      ...
      <div className="max-w-7xl mx-auto mb-48">
        <div className="mb-32">
          ...
          <TextReveal 
            ...
            className="text-6xl md:text-8xl font-serif text-white leading-[0.9] tracking-tighter premium-gradient-text"
          />
        </div>
            ...
            <div key={index} className={`grid lg:grid-cols-12 gap-20 items-center ${index % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
              <div className={`lg:col-span-7 ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
                <ImageReveal 
                  src={service.image} 
                  alt={service.title} 
                  className="aspect-[16/9] md:aspect-[21/9] editorial-shadow grayscale hover:grayscale-0 transition-all duration-700 rounded-2xl overflow-hidden border border-white/5"
                />
              </div>
              <div className={`lg:col-span-5 ${index % 2 === 1 ? 'lg:order-1' : ''}`}>
                <Reveal delay={0.2}>
                  <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center text-brand-blue-400 mb-10 border border-white/10 inner-glow-dark">
                    {service.icon}
                  </div>
                </Reveal>
                <Reveal delay={0.3}>
                ...
                <Reveal delay={0.5}>
                  <Magnetic strength={0.2}>
                    <button className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-[0.3em] text-white group">
                      Explore Procedure 
                      <div className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center group-hover:bg-white group-hover:text-brand-blue-900 transition-all duration-500 luxury-shadow">
        ...
        <div className="grid md:grid-cols-2 gap-16">
          {testimonials.map((testimonial, index) => (
            <Reveal key={index} delay={index * 0.2} y={30}>
              <div className="bg-white/5 backdrop-blur-md p-16 rounded-[2.5rem] border border-white/5 luxury-shadow h-full flex flex-col justify-between relative overflow-hidden group">
